...
<?php 
	...
	if(isset($_POST['edit']))
	{
		...
		if(($ekstensifile == "jpg") or ($ekstensifile == "JPG") or ($ekstensifile == "jpeg") or ($ekstensifile == "JPEG"))
		{
			...
			mysqli_query($connection, "update fotodestinasi set fotodestinasiNAMA = '$namafoto', fotodestinasiFILE = '$namafile', destinasiKODE = '$kodedestinasi' where fotodestinasiKODE = '$kodefoto'");

		}
        mysqli_query($connection, "update fotodestinasi set fotodestinasiNAMA = '$namafoto', destinasiKODE = '$kodedestinasi' where fotodestinasiKODE = '$kodefoto'");

	}

	$datadestinasi = mysqli_query($connection, "select * from destinasi");
    $inputkode = $_GET['ubahfoto'];
    $editfoto= mysqli_query($connection,"select * from fotodestinasi where fotodestinasiKODE = '$inputkode'");
    $rowedit = mysqli_fetch_array($editfoto);
    $editkode = mysqli_query($connection,"select * from fotodestinasi, destinasi where fotodestinasiKODE = '$inputkode' and fotodestinasi.destinasiKODE = destinasi.destinasiKODE");
...
<div class="col-sm-10">
	...
	<form method="POST" enctype="multipart/form-data">
		<div class="mb-3 row">
			...
			<div class="col-sm-10">
				<input type="text" class="form-control" id="kodefoto" name="inputkode" placeholder="Kode Photo" maxlength="4" value="<?php echo $rowedit["fotodestinasiKODE"]?>" readonly>
			</div>
		</div>
		...
		<div class="mb-3 row">
			...
			<div class="col-sm-10">
				<input type="text" class="form-control" id="namafoto" name="inputnama" placeholder="Nama Photo" value="<?php echo $rowedit["fotodestinasiNAMA"]?>">
			</div>
		</div>
		...
		<div class="mb-3 row">
			...
			<div class="col-sm-10">
				<input type="file" id="file" name="file" value="<?php echo $rowedit["fotodestinasiFILE"]?>">
	...
	<table class="table table-hover table-danger">
		...
		<tbody>
				...
				<tr>
					...
					<td>
						<a href="photodestinasiubah.php?ubahfoto=<?php echo $row['fotodestinasiKODE']?>" class="btn btn-success btn-sm" title="EDIT">
